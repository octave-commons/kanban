...
test('sync', async (t) => {
  ...
  t.deepEqual(result.tasks, { added: 0, moved: 1, statusUpdated: 1 });
...
test('archiveTask', async (t) => {
  ...
  t.true(getColumn(board, 'Archive').tasks.some((task) => task.uuid === 'todo-1'));
...
test('deleteTask', async (t) => {
  ...
  t.true(removed);
  t.false(getColumn(board, 'Todo').tasks.some((task) => task.uuid === 'todo-1'));
  const persisted = await getTaskFileByUuid(tasksDir, 'todo-1');
  t.falsy(persisted);
  const boardFile = await readFile(boardPath, 'utf8');
...
test('updateTaskDescription', async (t) => {
  ...
  t.truthy(updated);
  t.is(
    getColumn(board, 'Todo').tasks.find((task) => task.uuid === 'todo-1')?.content,
    'Updated task body',
  );
  const persisted = await getTaskFileByUuid(tasksDir, 'todo-1');
  t.truthy(persisted);
...
test('renameTask', async (t) => {
  ...
  const renamed = await renameTask(board, 'todo-1', 'Document architecture', tasksDir, boardPath);
