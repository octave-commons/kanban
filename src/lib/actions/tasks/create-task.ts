...
const debugCreateTask = (...params: unknown[]): void => {
  if (process.env.KANBAN_DEBUG_CREATE_TASK === '1') {
    console.log('[createTaskAction]', ...params);
  }
};
...
export const createTaskAction = async (config: TaskCreationConfig): Promise<TaskCreationResult> => {
  ...
  debugCreateTask('existing tasks', existingTasks.length);
  ...
  if (existingTaskInFiles) {
    debugCreateTask('found existing file task match, returning early');
    return { task: existingTaskInFiles, board };
  }

  debugCreateTask('dedup checks complete');
  const boardTaskInColumn = targetColumn.tasks.find(
    (task) => task.title.trim().toLowerCase() === normalizedTitle,
  );
  if (boardTaskInColumn) {
    debugCreateTask('found board column duplicate, returning existing task');
  ...
  debugCreateTask('board index built', boardIndex.size);

  const bodyText = input.body ?? input.content ?? '';
  debugCreateTask('processing template');
  ...
  debugCreateTask('template processed');
  ...
  debugCreateTask('slug assigned', uniqueSlug);
  ...
  debugCreateTask('writing board', boardPath);
