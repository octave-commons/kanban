...
test('EventLogManager records and retrieves transitions', async (t) => {
  ...
  const manager = makeEventLogManager(config);

  const taskId = randomUUID();
  await manager.logTransition(taskId, 'todo', 'in_progress', {
    actor: 'human',
    reason: 'started',
    metadata: {
      priority: 'P1',
  ...
  await manager.logTransition(taskId, 'in_progress', 'done', {
    actor: 'system',
  });
  ...
  t.deepEqual(new Set(allEvents.map((event) => event.taskId)), new Set([taskId]));
});
...
test('EventLogManager clears the log and reports statistics', async (t) => {
  ...
  await manager.logTransition(firstTask, 'incoming', 'accepted', {
    actor: 'human',
  });
  await manager.logTransition(secondTask, 'accepted', 'breakdown', {
    actor: 'agent',
  });
