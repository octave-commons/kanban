...
test.beforeEach(async (t) => {
  const testDir = await fs.mkdtemp(path.join(os.tmpdir(), 'scar-file-manager-'));
...
test('loadScars skips invalid records', async (t) => {
  ...
  const lines = [
    ...
    JSON.stringify({
      ...mockScarRecords[1]!,
      timestamp: mockScarRecords[1]!.timestamp.toISOString(),
    }),
    '{"incomplete": "record"}',
  ];
  await fs.writeFile(testFilePath, lines.join('\n') + '\n');
...
test('addScars rejects partially invalid records', async (t) => {
  ...
  const mixedRecords = [mockScarRecords[0]!, { invalid: 'record' } as any, mockScarRecords[1]!];
