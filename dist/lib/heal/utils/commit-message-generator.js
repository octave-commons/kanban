...
export class CommitMessageGenerator {
     ...
     */
    generatePreOperationMessage(context) {
     ...
     */
    generatePostOperationMessage(context, modifiedTasks) {
     ...
     */
    generateTasksDirectoryMessage(taskDiffs) {
        ...
        if (created.length > 0 && modified.length === 0 && deleted.length === 0) {
            ...
            const parts = [];
            if (created.length > 0)
                parts.push(`${created.length} added`);
            if (modified.length > 0)
                parts.push(`${modified.length} modified`);
            if (deleted.length > 0)
                parts.push(`${deleted.length} removed`);
     ...
     */
    generateKanbanBoardMessage(reason, modifiedTasks) {
     ...
     */
    generateDependenciesMessage(dependencies, operation) {
     ...
     */
    generatePreOperationBody(context) {
        const lines = [];
     ...
     */
    generatePostOperationBody(context, modifiedTasks) {
        const lines = [];
     ...
     */
    generateTasksDirectoryBody(taskDiffs) {
        const lines = [];
        const byType = {
     ...
     */
    generateKanbanBoardBody(modifiedTasks) {
        const lines = [];
        if (modifiedTasks.length === 0) {
     ...
     */
    generateDependenciesBody(dependencies, operation) {
        const lines = [];
     ...
     */
    generateScarNarrativeBody(scar) {
        const lines = [];
 ...
 */
export function createCommitMessageGenerator(options) {
