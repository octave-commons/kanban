...
import { createTransitionRulesEngineState, initializeTransitionRulesEngine, validateTransition as validateTransitionFn, getValidTransitions as getValidTransitionsFn, debugTransition as debugTransitionFn, showProcessFlow as showProcessFlowFn, validateTestingToReviewTransition as validateTestingToReviewTransitionFn, extractTestingInfo as extractTestingInfoFn, getTransitionsOverview as getTransitionsOverviewFn, normalizeColumnName, findTransitionRule, isBackwardTransition, evaluateGlobalRule as evaluateGlobalRuleFn, evaluateWipLimit, evaluateTaskExistence, evaluateCustomCheck as evaluateCustomCheckFn, evaluateCustomRule as evaluateCustomRuleFn, } from './transition-rules-functional.js';
/**
...
export class TransitionRulesEngine {
    ...
    getValidTransitions(from) {
        console.warn('TransitionRulesEngine.getValidTransitions is deprecated. Use getValidTransitions from transition-rules-functional instead.');
    ...
    async debugTransition(from, to, task, board) {
        console.warn('TransitionRulesEngine.debugTransition is deprecated. Use debugTransition from transition-rules-functional instead.');
     ...
     */
    showProcessFlow() {
        console.warn('TransitionRulesEngine.showProcessFlow is deprecated. Use showProcessFlow from transition-rules-functional instead.');
    ...
    getTransitionsOverview() {
        console.warn('TransitionRulesEngine.getTransitionsOverview is deprecated. Use getTransitionsOverview from transition-rules-functional instead.');
...
export async function createTransitionRulesEngine(configPaths) {
    ...
    throw new Error(`Failed to load transition rules configuration from any of the provided paths: ${paths.join(', ')}. Clojure DSL is required.`);
}
//# sourceMappingURL=transition-rules.js.map