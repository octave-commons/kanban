...
import { TransitionRulesEngine, type TransitionRulesConfig } from '../src/transition-rules.js';
import type { Board, Task } from '@promethean-os/kanban-sdk';
...
test.skip('TransitionRulesEngine requires Clojure DSL - throws error when DSL path missing', async (t) => {
...
test.skip('createTransitionRulesEngine fails fast without Clojure DSL - no fallback to TypeScript', async (t) => {
  await t.throwsAsync(
    async () => {
